...
class CalorieDAOTest {
    ...
    inner class ReadCalories {
        ...
        @Test
        fun `get calorieTime by calorieTime id that has no records, results in no record returned`() {
            transaction {
                //Arrange - create and populate tables with three users and three calories
                val userDAO = populateUserTable()
                val calorieDAO = populateCalorieTable()
    ...
    inner class UpdateCalories {
        ...
        fun `updating existing calorie in table results in successful update`() {
            transaction {
                ...
                //Act & Assert
                val calorie3updated = Calorie(id = 3, dateTime = DateTime.now(), calorieGet = 4920, state = "Over", userId = 2)
        ...
        fun `updating non-existant calorie in table results in no updates`() {
            transaction {
                ...
                //Act & Assert
                val calorie4updated = Calorie(id = 3, dateTime = DateTime.now(), calorieGet = 4920, state = "Over", userId = 2)
